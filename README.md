...
6. [Demo Login (Test Accounts)](#-demo-login-test-accounts)
7. [How to Try Every Feature](#-how-to-try-every-feature)
...
8. [How It Works (Technical)](#-how-it-works-technical)
...
The UI is designed for senior citizens: large type, high-contrast cards, one action per screen,
and every screen available in Hindi as well as English.

<table>
  <tr>
    <td align="center"><b>SMS Protection</b> (हिंदी)</td>
    <td align="center"><b>Settings — Language & Permissions</b> (हिंदी)</td>
  </tr>
  <tr>
    <td><img src="screenshots/sms-protection-hindi.jpg" width="300" alt="SMS Protection screen in Hindi showing protection status, enable SMS protection button and how protection works"></td>
    <td><img src="screenshots/settings-language-hindi.jpg" width="300" alt="Settings screen in Hindi showing language selector and protection permissions"></td>
  </tr>
  <tr>
    <td>Protection status card, one-tap permission request, and a 3-step explanation of how automatic scanning works.</td>
    <td>Language switcher (applies to every screen, notification and voice message) and a live view of required permissions.</td>
  </tr>
</table>

**Demo details:** an installable APK is linked at the top of this page, demo login numbers are in
[Demo Login](#-demo-login-test-accounts), and ready-to-send test messages with expected results are in
[How to Try Every Feature](#-how-to-try-every-feature).
...
| Layer | Technology |
|---|---|
| **Language** | Kotlin (Android), JavaScript (UI logic) |
| **Platform** | Android SDK — `minSdk 26` (Android 8.0) · `targetSdk 37` |
| **UI** | Android WebView rendering HTML5 / CSS3 / vanilla JavaScript from `assets/` — no UI framework, no external CDN |
| **Native ↔ UI bridge** | `addJavascriptInterface` (`KavachAndroid`, `KavachReport`) |
| **Authentication** | Firebase Authentication — Phone / OTP (`firebase-auth 24.x`), AndroidX Credentials |
| **SMS interception** | `BroadcastReceiver` on `android.provider.Telephony.SMS_RECEIVED` |
| **APK analysis** | `PackageManager.getPackageArchiveInfo()` static analysis, `MessageDigest` SHA-256, `FileProvider` hand-off to the package installer |
| **Storage** | SQLite (`SQLiteOpenHelper`) for threat history · `SharedPreferences` for session, language and trusted contact |
| **Alerts** | Android `NotificationManager` (high-importance channel) · `SmsManager` for trusted-contact SMS |
| **Voice** | Android `TextToSpeech` (`en-IN`, `hi-IN`) |
| **Libraries** | AndroidX AppCompat, Activity-KTX, Core-KTX, ConstraintLayout, Material Components |
| **Build** | Gradle 9.6 · Android Gradle Plugin 9.4 · Kotlin DSL · version catalog (`libs.versions.toml`) · Google Services plugin |
